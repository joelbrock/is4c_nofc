...
$noninventoryDeptQ = "SELECT d.department,t.dept_name,ROUND(sum(total),2) as total 
	...
	WHERE d.department = t.dept_no
	AND date(d.datetime) = '$db_date'
	AND d.department >= 33 AND d.department <= 44
	AND d.trans_status <> 'X'
	AND d.emp_no <> 9999
	GROUP BY t.dept_no
	ORDER BY t.dept_no";
...
extract(MADcoupon($table, $db_date, $db_date));  			//  we use compact+extract here to return mult. values from function
extract(foodforall($table, $db_date, $db_date));			//  we use compact+extract here to return mult. values from function
...
// 
// 	$staffMADQ = "SELECT SUM(unitprice) AS wmFFA
// 		FROM is4c_log.$table
// 		WHERE DATE(datetime) BETWEEN '$date1' AND '$date2'
// 		AND staff IN (1,2)
// 		AND voided = 9
...
// 
// 	$staffSSDDQ = "SELECT SUM(regPrice) AS ssdd
// 		FROM is4c_log.$table
// 		WHERE DATE(datetime) BETWEEN '$date1' AND '$date2'
// 		AND staff IN (1,2)
// 		AND voided = 22
// 		AND trans_status <> 'X'
// 		AND emp_no <> 9999";
// 	$staffSSDDR = mysql_query($staffSSDDQ);
// 	list($staffSSDD) = mysql_fetch_row($staffSSDDR);
// 	$staffSSDD = (is_null($staffSSDD) ? 0 : $staffSSDD);
// 	
// 	$staff_total -= $staffFFA;
// 	$staff_total -= $staffMAD;
// 	$staff_total -= $staffSSDD;
// 
include('net.php');
// $staff_total = $staff_base - $staffFFA - $staffMAD - $staffSSDD; 
// echo "staff= " . $staff_total;
...
echo "<tr><td>patronage refunds</td><td align=right>".money_format('%n',$pt_total)."</td></tr>";
...
echo "<tr><td>D2 (for kris)</td><td align=right>". money_format('%n',$d2) ."</td></tr></table>";
echo '------------------------------<br>';
...
// $staff_total = $staff_base - $staffFFA - $staffMAD - $staffSSDD;
// echo "<tr><td colspan=2>STAFF TOTAL ( ".$staff_base."base - ".$staffFFA."ffa - ".$staffMAD."mad - ".$staffSSDD."ssd == ".$staff_total.")</td></tr>";
...
if (strtotime($db_date) <= strtotime($dbChangeDate)) {
	echo "<tr><td>Uncaught Discount/FFA</td><td align=right>".money_format('%n',$miscDisc)."</td></tr>";
} else {
	echo "<tr><td>Manual Member Discounts</td><td align=right>".money_format('%n',$miscDisc)."</td></tr>";
}
