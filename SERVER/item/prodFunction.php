...
function itemParse($upc){
    if (is_numeric($upc)) {
        ...
        $queryItem = "SELECT p.upc AS upc, 
				p.description AS description, 
				p.normal_price AS normal_price, 
				p.special_price AS special_price, 
				p.discounttype AS discounttype 
			FROM " . PRODUCTS_TBL . " p, product_details d 
			WHERE p.upc = d.upc 
			AND ((p.description LIKE '%$upc%') OR (d.description LIKE '%$upc%') OR (d.brand LIKE '%$upc%'))
			GROUP BY p.upc ORDER BY p.description";
    }
...
function itemMaintenance($new,$input) {
	if ($new != 1) {
        ...
        // $queryItem = "SELECT p.*, d.cost as cost, d.brand as brand, d.upc FROM " . PRODUCTS_TBL . " p, product_details d WHERE p.upc = d.upc AND p.upc = '$upc'";
		$queryItem = "SELECT p.upc AS upc, 
				p.description AS description, 
				p.normal_price AS normal_price, 
				p.special_price AS special_price, 
				...
				p.foodstamp AS foodstamp,
				p.wicable AS wicable,
				p.qttyEnforced AS qttyEnforced,
	...
	// $disc = ($new != 1) ? "value='" . $rowItem['discount'] . "'" : "1";
	$strdate = strtotime($rowItem['end_date']);
	$enddate_long = strftime('%A %B %e, %Y',$strdate);
	...
	echo "<tr><td><b>Description</b></td>\n
		...
		<td align='right'><font size=1><b>Price $</b></font></td>\n
		<td><input type=text $pric name=price size=6></td></tr>";
	if($new != 1 && $rowItem['special_price'] <> 0){
		echo "<tr><td><font color=green>End Date:</td>\n<td><font color=green>" . $enddate_long . "</font></td>\n<td>";
		echo "<font color=green><b>Sale Price:</b></font></td>\n<td><font color=green>" . $rowItem['special_price'] . "</font></td><tr>\n";
	}
	echo "<tr><td align=right><b>Brand</b></td>
