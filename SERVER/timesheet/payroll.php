...
// mysql_select_db($db_slave, 'is4c_log');
	...
	$hours = array();
	    ...
	    $yearEnd = substr($periodEnd, 0, 4);
            ...
            $prevQ = "SELECT SUM(ROUND(TIMESTAMPDIFF(MINUTE, time_in, time_out)/60,2)), date, DAYOFWEEK(date)
                ...
                GROUP BY date";
            $prevR = mysql_query($prevQ);

            if (mysql_num_rows($prevR) > 0) {
                $totalPHours[$emp_no] = 0;
                $totalPOT[$emp_no] = 0;

                while (list($pHours, $pDate, $pDay) = mysql_fetch_row($prevR)) {
                    ...
                    list($weekStart, $weekEnd) = mysql_fetch_row($weekR);
                    ...
                    list($totalHours) = mysql_fetch_row($R);
