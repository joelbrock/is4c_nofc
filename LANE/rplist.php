...
for ($i = 0; $i < $num_rows; $i++) {
	...
	echo "><font size='+2'>".$row["emp_no"]."-".$row["trans_no"]."-".$row["register_no"]." -- ".substr($row["time"],10)." -- $".number_format(($row["total"]),2);
	$selected = "";
}
...
<TD width='240'>
<FONT face='arial' size='' color='#96C8ED'>use arrow keys to navigate<BR>[enter] to reprint receipt<BR>[c] to cancel</FONT>
</TD>
</TR>
</TABLE>
