...
//
		...
		$pattern = '/%LOCAL%/';
		$replace = "Congrats! X% of your groceries are local";

		$rf1 = $_SESSION["receiptFooter1"];
		$rf2 = $_SESSION["receiptFooter2"];
		$rf3 = $_SESSION["receiptFooter3"];
		$rf4 = $_SESSION["receiptFooter4"];
		if (preg_match($pattern, $rf1)) {
			$_SESSION["receiptFooter1"] = preg_replace($pattern, $replace, $rf1);
		}
		if (preg_match($pattern, $rf2)) {
			$_SESSION["receiptFooter2"] = preg_replace($pattern, $replace, $rf2);
		}
		if (preg_match($pattern, $rf3 == 1)) {
			$_SESSION["receiptFooter3"] = preg_replace($pattern, $replace, $rf3);
		}
		if (preg_match($pattern, $rf4)) {
			$_SESSION["receiptFooter4"] = preg_replace($pattern, $replace, $rf4);
		}
//
//		/////////////////////
//
