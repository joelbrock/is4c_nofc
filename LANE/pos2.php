...
if (!function_exists("trackPatronage")) include_once("prehkeys.php");
...
<HEAD>
<TITLE></TITLE>
<BODY background='graphics/is4c_bg.gif'>
...
elseif ($_SESSION["plainmsg"] && strlen($_SESSION["plainmsg"]) > 0) {
	...
	elseif (strlen($entered) == 2) {
		...
		switch ($entered) {
			case "PT":
				boxMsg("patronage tender must specify amount");
				break;
			case "ES":
				endofShift();
				break;
...
//					boxMsg("This discount is for <br>members only.");
//					break;
//				}
				...
				if ($_SESSION['MADdiscount'] == 0) {
					boxmsg("MAD Coupon is not <br>currently applicable.");
					break;
				}
				if ($_SESSION['madCoup'] == 0) {
					if ($_SESSION["tenderTotal"] != 0) {
						boxMsg("discount not applicable after tender");
						break;
					} 
					else {
						$existingPD = $_SESSION["percentDiscount"];
						$stackablePD = $_SESSION["MADdiscount"];
//						$equivalentPD = 100 - ((100 - $existingPD) * (100 - $stackablePD) / 100);	//	Compunding discounts
						...
						MADdiscount($equivalentPD);
						$_SESSION["differencePD"] = ($equivalentPD - $existingPD);

						$_SESSION['madCoup'] = 1;
					}
				} 
				else {
					boxMsg("Sorry, only one<br>".$_SESSION['MADdiscount']."% member coupon<br> per transaction please.");
					break;
				}				
				break;
				case "SD":
				if ($_SESSION['isStaff'] == 0) {
					boxMsg("This discount is for <br>STAFF MEMBERS only.");
					break;
				}
				if ($_SESSION["memberID"] == "0") {
					maindisplay("memsearch.php");
				...
				//if ($_SESSION['madCoup'] == 0) {
					if ($_SESSION["tenderTotal"] != 0) {
						...
						$existingPD = $_SESSION["percentDiscount"];
						$stackablePD = $_SESSION["SSDdiscount"];
		$equivalentPD = 100 - ((100 - $existingPD) * (100 - $stackablePD) / 100); //	Compunding discounts
		$equivalentPD = ($existingPD + $stackablePD);		//	sum discounts
						SSDdiscount($equivalentPD);
						$_SESSION["differencePD"] = ($equivalentPD - $existingPD);
			...
			case "FF":
				if ($_SESSION['isMember'] == 0) {
					xboxMsg("This discount is for <br>members only.");
					break;
				}
				if ($_SESSION['needBasedDisc'] == 0) {
					boxmsg("Discount is not <br>currently applicable.");
					break;
				}
				if ($_SESSION['NBdisc'] == 0) {
					if ($_SESSION["tenderTotal"] != 0) {
						boxMsg("discount not applicable after tender");
						break;
					}
						...
						$existingPD = $_SESSION["percentDiscount"];
						$stackablePD = $_SESSION["needBasedDisc"];
//						$equivalentPD = 100 - ((100 - $existingPD) * (100 - $stackablePD) / 100);	//	Compunding discounts
						$equivalentPD = ($existingPD + $stackablePD);								//	sum discounts
						needBasedDisc($equivalentPD);
						$_SESSION["differencePD"] = ($equivalentPD - $existingPD);
					...
					boxMsg("Sorry, only one<br>".$_SESSION['needBasedDisc']."% FoodForAll discount<br> per transaction please.");
					break;
				}
				break;
			case "BQ":
...
*/			/*	if ($_SESSION["memberID"] == 0) {
					...
					$_SESSION["PRamt"] = $strl;	
					$intAway = 1;
					maindisplay("patronageselect.php");
				}
				break;		*/
// --------------------------------------------			
			case "ID":
