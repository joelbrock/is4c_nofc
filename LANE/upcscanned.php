...
function upcscanned($entered) {
	...
	if ($_SESSION["store"] == 'nofc' && substr($entered,0,6) == "000999") { 
		// *** SCANNABLE MEMBER CARDS MODULE ***
		// Searches input for mem# prefix "000999", takes the rest as the member#
		// ex. "00099900234" == mem# 234
		$card_no = substr($entered,6,5);
		$memIDR = sql_query("SELECT id FROM custdata WHERE cardNo = $card_no", $db);
		$memID = sql_fetch_row($memIDR);
		setMember($memID[0]);
		lastpage();
		// *** END SCANNABLE MEMBER CARDS MODULE ***
	} else {
		$query = "select * from opdata.products where upc = '".$upc."' AND inUse = 1";
...
*/	
			...
			if ($_SESSION["qttyvalid"] != 1) sql_close($db);
				...
				elseif ($_SESSION["store"] == "pfc" && $discounttype == 2) {
					$unitPrice = $special_price;
					$unitDiscount = $normal_price - $special_price;
					$discount = $unitDiscount * $quantity;
					$memDiscount = 0;
	...
	//-------------Mix n Match -------------------------------------
				...
				if ($volDiscType && $volDiscType >= 1) {			// If item is on volume discount
					...
					if ($newmm >= 1) {
						addItem($upc, $description, "I", "", "", $department, $cost, $newmm, truncate2($VolSpecial), truncate2($newmm * $VolSpecial), truncate2($VolSpecial), $scale, $tax, $foodstamp, $discount, $memDiscount, $discountable, $discounttype, $volume * $newmm, $volDiscType, $volume, $VolSpecial, $mixMatch, $volume * $newmm, 0, 0, '', $row['props']);
						$newmm = 0;
						$_SESSION["qttyvalid"] = 0;
					}

					if ($newmmtotal >= $volume) {
						addItem($upc, $description, "I", "", "", $department, $cost, 1, $voladj, $voladj, $voladj, $scale, $tax, $foodstamp, $discount, $memDiscount, $discountable, $discounttype, 1, $volDiscType, $volume, $VolSpecial, $mixMatch, $volume, 0, 0, '', $row['props']);
						$quantity = $quantity - 1;
						$newmmtotal = 0;
						$_SESSION["qttyvalid"] = 0;
					}
...
*/
				if ($hitareflag == 1) {
					...
					if ($quantity != 0) {
						...
						addItem($upc, $description, "I", " ", " ", $department, $cost, $quantity, $unitPrice, $total, $regPrice, $scale, $tax, $foodstamp, $discount, $memDiscount, $discountable, $discounttype, $qtty, $volDiscType, $volume, $VolSpecial, $mixMatch, $matched, $intvoided, 0, 0, '', $row["props"]);
